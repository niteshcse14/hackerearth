...
int get_ODD_EVEN(int BITree[], int index, int type)
{
	int count_ODD = 0, count_EVEN = 0, sum = 0;
	index += 1;
	while(index > 0)
	{
		//sum += BITree[index];
...
/*
int sum(int x, int BITree1[], int BITree2[], int type)
{
	return get_ODD_EVEN(BITree1, x, type) * x - get_ODD_EVEN(BITree2, x, type);
}
int range_ODD_EVEN(int BITree1[], int BITree2[], int l, int r, int type)
{
	return sum(r, BITree1, BITree2, int type) - sum(l - 1, BITree1, BITree2, type);
}
*/
...
void solve()
{
	...
	while(Q--)
	{
		...
		{
			p_i(get_ODD_EVEN(BITree, x - 1, type));
		}
		FOR_A(i, 0, N + 1)
		{
			cout<<BITree[i]<<"  ";
